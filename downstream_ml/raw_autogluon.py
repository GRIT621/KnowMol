...
def extract_raw_features(data: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    ...
    if missing:
        raise ValueError(f"Raw-AutoGluon data is missing required columns: {missing}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Raw-AutoGluon baseline using only raw drug SMILES and target protein sequences"
    )
    subparsers = parser.add_subparsers(dest="mode", required=True)

    train = subparsers.add_parser("train", help="Train Raw-AutoGluon on drug,target only")
    ...
    test = subparsers.add_parser("test", help="Evaluate a saved Raw-AutoGluon model")
